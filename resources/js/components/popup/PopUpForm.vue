<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-sm" @click.self="$emit('close')">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
            <h3 class="mb-2 text-2xl font-bold text-gray-900">Предварительная регистрация</h3>
            ...
            <form @submit.prevent="submitForm" class="space-y-4">
                <div>
                    <label for="name" class="mb-1 block font-medium text-gray-900">Ваше имя или название компании *</label>
                    <input
                        id="name"
                        v-model="form.name"
                        type="text"
                        class="w-full rounded border border-gray-300 px-3 py-2 text-gray-900 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    />
                    <p v-if="form.errors.name" class="mt-1 text-sm text-red-600">{{ form.errors.name }}</p>
                </div>

                <div>
                    <label for="email" class="mb-1 block font-medium text-gray-900">Email *</label>
                    <input
                        id="email"
                        v-model="form.email"
                        type="email"
                ...
                <div>
                    <label for="phone" class="mb-1 block font-medium text-gray-900">Номер телефона (с кодом страны)</label>
                    <input
                        id="phone"
                        v-model="form.phone"
                        type="tel"
                ...
                <div>
                    <label for="region" class="mb-1 block font-medium text-gray-900">Город / Регион</label>
                    <input
                        id="region"
                        v-model="form.region"
                        type="text"
                ...
                <div class="flex items-start">
                    ...
                    <label for="agree" class="text-sm text-gray-900"> Я соглашаюсь на обработку персональных данных в соответствии с ФЗ-152 * </label>
                </div>
