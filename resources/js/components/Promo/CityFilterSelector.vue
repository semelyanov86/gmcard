    ...
    <div class="gapper filter_inp relative flex items-center gap-6 lg:flex-col">
        ...
        <div class="selected_block relative inline-block h-[46px]">
            <div
                ...
                class="custom-select focus:shadow-outline flex h-[46px] w-[202px] cursor-pointer appearance-none items-center rounded-md border border-white bg-none px-4 py-2 pr-8 leading-tight text-white shadow hover:border-gray-300 focus:outline-none"
            >
                <span class="mr-2">{{ selectedCity }}</span>
                <ChevronDownIcon custom-class="pointer-events-none absolute top-2 right-0 mt-3 mr-2 h-[5px] w-2 text-white" />
            </div>
            <div
                v-show="isOpen"
                class="custom-options absolute z-50 mt-1 w-[202px] overflow-hidden rounded-b border border-gray-400 bg-white shadow-lg"
            >
                <input
                    v-model="searchQuery"
                    type="text"
                ...
                <div class="max-h-[180px] overflow-y-scroll">
                    <div
                        ...
                        :class="{ 'bg-[#F9D914]': city.name === selectedCity }"
                    >
                        {{ city.name }}
                    </div>
                    <div
